...
def contrast_edge_color_sobel(image):
    """
...
def symmetry_superpixel(image, num_segments=40, symmetry_axis='vertical', min_region_size=30,threshold=0.55):
    image_float = img_as_float(image)
...
def symmetry_superpixel_preproc(image, num_segments=40, symmetry_axis='vertical', min_region_size=30,threshold=0.55):
    image_float = img_as_float(image)
    #segments = slic(image_float, n_segments=num_segments, compactness=10, sigma=4, start_label=1)
...
def symmetry_seeded_region(image, symmetry_axis='vertical', min_region_size=30, sigma=5,threshold=0.55):
    """
...
def symmetry_felzenszwalb(image, scale=100, sigma=3, min_size=100, symmetry_axis='vertical',
...
def symmetry_watershed(image, symmetry_axis='vertical', min_region_size=100, sigma_blur=4,threshold=0.55):
    """
...
def symmetry_superpixel_h(image, num_segments=40, symmetry_axis='horizontal', min_region_size=30,threshold=0.55):
    image_float = img_as_float(image)
    segments = slic(image_float, n_segments=num_segments, compactness=10, sigma=4, start_label=1)
...
def symmetry_superpixel_preproc_h(image, num_segments=40, symmetry_axis='horizontal', min_region_size=30,threshold=0.55):
    image_float = img_as_float(image)
    #segments = slic(image_float, n_segments=num_segments, compactness=10, sigma=4, start_label=1)
    segments = preprocess_and_segment_superpixels(image, num_segments=60, compactness=10, sigma=1)
    output_mask = np.zeros(image.shape[:2], dtype=np.uint8)
...
def symmetry_seeded_region_h(image, symmetry_axis='horizontal', min_region_size=30, sigma=5,threshold=0.55):
    """
    Region symmetry detection using seeded region growing from intensity maxima.
    """
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
...
def symmetry_felzenszwalb_h(image, scale=100, sigma=3, min_size=100, symmetry_axis='horizontal',
                                     min_region_size=100,threshold=0.55):
    """
    Detect symmetric regions using Felzenszwalb segmentation.
...
def symmetry_watershed_h(image, symmetry_axis='horizontal', min_region_size=100, sigma_blur=4,threshold=0.55):
    """
    Detect symmetric regions using Watershed segmentation.
...
HEURISTIC_FUNCTIONS = {
    # --- Contrast ---
    "contrast_edge_luminance_laplacian": contrast_edge_luminance_laplacian,
    "contrast_edge_luminance_sobel": contrast_edge_luminance_sobel,
    "contrast_edge_color_sobel": contrast_edge_color_sobel,
    "contrast_edge_color_gradient": contrast_edge_color_gradient,
    "contrast_region_color_slic": contrast_region_color_slic,

    "contrast_texture_gabor": contrast_texture_gabor,
    "contrast_texture_lbp": contrast_texture_lbp,
    "contrast_texture_entropy": contrast_texture_entropy,

    # --- Symmetry (Vertical) ---
    "symmetry_watershed": symmetry_watershed,
    "symmetry_felzenszwalb": symmetry_felzenszwalb,
    "symmetry_seeded_region": symmetry_seeded_region,
    "symmetry_superpixel_preproc": symmetry_superpixel_preproc,
    "symmetry_superpixel": symmetry_superpixel,

    # --- Symmetry (Horizontal) ---
    "symmetry_watershed_h": symmetry_watershed_h,
    "symmetry_felzenszwalb_h": symmetry_felzenszwalb_h,
    "symmetry_seeded_region_h": symmetry_seeded_region_h,
    "symmetry_superpixel_preproc_h": symmetry_superpixel_preproc_h,
    "symmetry_superpixel_h": symmetry_superpixel_h,

    # --- Isolation: LAB ---
    "isolation_color_global_abs": isolation_color_global_abs,
    "isolation_color_local_abs": isolation_color_local_abs,
    "isolation_color_local_multi": isolation_color_local_multi,

    # --- Isolation: Intensity ---
    "isolation_intensity_global_abs": isolation_intensity_global_abs,
    "isolation_intensity_local_abs": isolation_intensity_local_abs,
    "isolation_intensity_local_multi": isolation_intensity_local_multi,

    # --- Isolation: Texture Entropy ---
    "isolation_tex_entropy_global_abs": isolation_tex_entropy_global_abs,
    "isolation_tex_entropy_local_abs": isolation_tex_entropy_local_abs,
    "isolation_tex_entropy_local_multi": isolation_tex_entropy_local_multi,

    # --- Isolation: Texture LBP ---
    "isolation_tex_lbp_global_abs": isolation_tex_lbp_global_abs,
    "isolation_tex_lbp_local_abs": isolation_tex_lbp_local_abs,
    "isolation_tex_lbp_local_multi": isolation_tex_lbp_local_multi,

    # --- Isolation: Texture Gabor ---
    "isolation_tex_gab_global_abs": isolation_tex_gab_global_abs,
    "isolation_tex_gab_local_abs": isolation_tex_gab_local_abs,
    "isolation_tex_gab_local_multi": isolation_tex_gab_local_multi,

    # --- Grouping ---
    "grouping_color_histogram_bins": grouping_color_histogram_bins,
    "grouping_line_hough": grouping_line_hough,
}
